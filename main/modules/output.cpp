...
static Module_ptr create_output(const std::string &name, const std::vector<ConstExpression_ptr> &arguments, MessageHandler) {
    if (arguments.size() == 1) {
...
REGISTER_MODULE(Output, &create_output)
...
Output::Output(const std::string name) : Module(name) {
