...
ExternalProxy::ExternalProxy(const std::string name,
                             const std::string expander_name,
                             const std::string module_type,
                             const ExternalExpander_ptr external_expander,
                             const std::vector<ConstExpression_ptr> arguments)
    : Module(external_proxy, name), external_expander(external_expander) {
...
void ExternalProxy::call(const std::string method_name, const std::vector<ConstExpression_ptr> arguments) {
    static char buffer[256];
    int pos = std::sprintf(buffer, "%s.%s(", this->name.c_str(), method_name.c_str());
    pos += write_arguments_to_buffer(arguments, &buffer[pos]);
    pos += std::sprintf(&buffer[pos], ")");
    this->external_expander->serial->write_checked_line_id(this->external_expander->device_id, buffer, pos);
}

void ExternalProxy::write_property(const std::string property_name, const ConstExpression_ptr expression, const bool from_expander) {
