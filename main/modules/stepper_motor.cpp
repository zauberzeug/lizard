...
void StepperMotor::set_state(StepperState new_state) {
    ...
    gpio_matrix_out(this->step_pin, new_state == Idle ? SIG_GPIO_OUT_IDX : LEDC_LS_SIG_OUT0_IDX + this->ledc_channel, 0, 0);
    ledc_set_duty(LEDC_LOW_SPEED_MODE, this->ledc_channel, new_state == Idle ? 0 : 1);
    ledc_update_duty(LEDC_LOW_SPEED_MODE, this->ledc_channel);
}
...
void StepperMotor::step() {
    ...
    if (this->state != Idle) {
        ...
        if (abs_speed < MIN_SPEED) {
            ledc_timer_pause(LEDC_LOW_SPEED_MODE, this->ledc_timer);
        } else {
            ledc_set_freq(LEDC_LOW_SPEED_MODE, this->ledc_timer, abs_speed);
            ledc_timer_resume(LEDC_LOW_SPEED_MODE, this->ledc_timer);
        }
