...
# calls, section placement).
#
# Usage:
#   release-notes.sh dossier "<milestone>"            # full JSON dossier (default)
#   release-notes.sh verify  "<milestone>" [file]     # milestone set vs #s in file (default tmp.md)
#   release-notes.sh check-docs <slug> [slug...]      # HTTP status for each docs page
#
# Override the repo via env if ever reused elsewhere:
#   RELEASE_NOTES_REPO=owner/repo
set -euo pipefail

REPO="${RELEASE_NOTES_REPO:-zauberzeug/lizard}"
...
dossier() {
  ...
  jq -n --arg ms "$milestone" --argjson tickets "$tickets" --argjson cross "$cross" \
    '{milestone: $ms, tickets: $tickets, cross_references: $cross}'
}

verify() {
  local milestone="${1:?milestone required}" file="${2:-tmp.md}"
...
check_docs() {
  ...
  local slug code
  for slug in "$@"; do
    code=$(curl -s -o /dev/null -w '%{http_code}' -L "https://lizard.dev/$slug/")
    printf '%s -> %s\n' "$slug" "$code"
  done
}
