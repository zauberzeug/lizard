...
def perform_ota(dev: serial.Serial, firmware_path: str, target: int, chunk_size: int, window: int) -> bool:
...
def main() -> int:
    ...
    try:
        with serial.Serial(args.port, args.baud, timeout=0.5) as dev:
            ...
            success = perform_ota(dev, args.firmware, args.id, args.chunk_size, args.window)
