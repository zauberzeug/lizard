...
When the motor is disabled, it will stop and ignore in/out commands.
...
It supports torque-based gripping (FOC mode) and position control for releasing.

| Constructor                              | Description                    | Arguments         |
| ---------------------------------------- | ------------------------------ | ----------------- |
| `motor = MksServoMotor(can, can_id)` | CAN module and slave CAN ID | CAN module, `int` |

| Properties       | Description                                | Data type |
| ---------------- | ------------------------------------------ | --------- |
| `motor.status`   | Motor operating status code                | `int`     |
| `motor.position` | Multi-turn encoder position                | `float`   |
| `motor.speed`    | Current speed (RPM)                        | `float`   |
| `motor.can_age`  | Time since last CAN message from motor (s) | `float`   |
| `motor.enabled`  | Whether the motor is enabled               | `bool`    |

Status codes for `motor.status` depend on the last command. For `query_status()`: 0=query failed, 1=stop, 2=speed up, 3=speed down, 4=full speed, 5=homing, 6=calibration. For motion commands: 0=fail, 1=starting, 2=complete, 3=end limit stopped.

| Methods                                        | Description                                              | Arguments           |
| ---------------------------------------------- | -------------------------------------------------------- | ------------------- |
| `motor.set_mode(mode)`                         | Set working mode (see table below)                       | `int`               |
| `motor.set_working_current(current)`           | Set working current in mA                                | `int`               |
| `motor.set_holding_current(ratio)`             | Set holding current percentage (0=10%, 1=20%, ... 8=90%) | `int`               |
| `motor.move_to_position(speed, acc, pulses)`   | Move to absolute pulse position                          | `int`, `int`, `int` |
| `motor.run_speed(direction, speed, acc)`       | Run at given speed (RPM) with acceleration               | `int`, `int`, `int` |
| `motor.grip(current, ratio)`                   | Set FOC mode, working current (mA) and holding ratio     | `int`, `int`        |
| `motor.home(current, ratio)`                   | Home sequence: grip, release, wait for springs, set zero  | `int`, `int`        |
| `motor.stop()`                                 | Emergency stop                                           |                     |
| `motor.query_status()`                         | Query motor operating status                             |                     |
| `motor.read_encoder()`                         | Read multi-turn encoder value                            |                     |
| `motor.read_speed()`                           | Read real-time speed (RPM)                               |                     |
| `motor.clear_stall()`                          | Release motor from stall protection state                |                     |
| `motor.set_zero()`                             | Set current position as zero point                       |                     |
| `motor.enable()`                               | Enable the motor                                         |                     |
| `motor.disable()`                              | Disable the motor                                        |                     |

When the motor is disabled, it will stop and the shaft will be released.

**Working modes**

| Mode | Name      | Description                       |
| ---- | --------- | --------------------------------- |
| 0    | CR_OPEN   | Pulse interface open-loop mode    |
| 1    | CR_CLOSE  | Pulse interface closed-loop mode  |
| 2    | CR_vFOC   | Pulse interface FOC mode          |
| 3    | SR_OPEN   | Bus interface open-loop mode      |
| 4    | SR_CLOSE  | Bus interface closed-loop mode    |
| 5    | SR_vFOC   | Bus interface FOC mode            |

Bus control commands (`run_speed`, `move_to_position`, `grip`, `home`) require a bus mode (3, 4 or 5).

**Grip and home**

The `grip()` method is a convenience wrapper that sets the motor to SR_vFOC mode (5), configures the working current and holding current ratio in one call.

The `home()` method performs a spring-return homing sequence:

1. Grip with the given current and holding ratio (motor closes)
2. Wait 1 second for grip to engage
3. Disable motor (release shaft so springs pull back to base)
4. Wait 2 seconds for springs to settle
5. Set current position as zero point
6. Re-enable motor

**Speed mode parameters**

For `run_speed(direction, speed, acceleration)`:
...
For `move_to_position(speed, acceleration, pulses)`:

- `speed`: 0-3000 RPM
- `acceleration`: 0-255
- `pulses`: absolute pulse count (int24, -8388607 to +8388607). One revolution at 16 microsteps = 3200 pulses.

**Working current limits**

- SERVO42D/28D/35D: max 3000 mA
- SERVO57D: max 5200 mA
